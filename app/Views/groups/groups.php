...
<script>
function ToSearch(){
	var vGroupName	= $('#vGroupName').textbox('getValue');
	var vDescription	= $('#vDescription').textbox('getValue');
...
function createUser(){
	$('#dialog-form').dialog({
		...
		iconCls:'icon-add',
		title:'Add Data',
		href:'../groups/formGroups',
		onLoad:function(){
			$('#form').form('disableValidation');
			url = "<?php echo base_url('groups/createGroups'); ?>";
		}
	...
	if(row){
		...
		$('#dialog-form').dialog({
			...
			onLoad:function(){
				...
				$('#form').form('load',row);
				$('#form').form('disableValidation');
				url = "<?php echo base_url('groups/updateGroups'); ?>";
			}
		});
...
function removeUser(){
	...
	if (row){
		jQuery.messager.confirm('Confirm','Are you sure you want to remove ?',function(r){
			if (r){
				$.ajax({
					url: "<?php echo base_url('groups/deleteGroups'); ?>",
					dataType: 'json',
					type: 'post',
...
<div data-options="region:'west',split:true" title="Look up &amp; Tools" style="width:200px;">
		<table width="180" border="0" cellspacing="1" cellpadding="2" align="center">
			...
			<tr>
				<td height="20" valign="middle" align="right" style="font-size:12px;">Group Name</td>
				<td width="100" height="20" valign="middle" align="left"><input  Name="vUserName" id="vGroupName" size="8" value="" type="text" class="easyui-textbox"/></td>
			</tr>
			
			<tr>
				<td height="20" valign="middle" align="right" style="font-size:12px;">Description</td>
				<td width="100" height="20" valign="middle" align="left"><input  Name="vUserName" id="vDescription" size="8" value="" type="text" class="easyui-textbox"/></td>
			</tr>
...
<!-- #Body -->
<div data-options="region:'center',title:'Master Groups'" style="background-color:#D7E4F2;">
	<!-- TABLE UTAMA -->
	<div style="height:75%" bgcolor="#3E6DB9">
	<table id="datagrid-master" class="easyui-datagrid" style="height:100%" url="<?php echo base_url('groups/getJson'); ?>" fit="true" toolbar="#toolbar-master" pagination="true" rownumbers="true" fitColumns="false" singleSelect="true" collapsible="true">
		<thead>
			<tr>
				<th field="groupId" width="120" sortable="true">ID</th>
