...
function approvalSalesOrders(){
	var row = jQuery('#datagrid-master').datagrid('getSelected');
	if (row){
		jQuery.messager.confirm('Confirm','Are you sure ?',function(r){
			if (r){
				$.ajax({
					url: "<?php echo base_url('salesorders/approvalSalesOrders'); ?>",
					dataType: 'json',
					type: 'post',
					// contentType: 'application/json',
					data: { "salesOrderId": row.salesOrderId, "approvalId": '<?php echo $approvalId ?>', "approvalHeader": '<?php echo $approvalHeader ?>' },
					processData: true,
...
<div id="p" class="easyui-panel" style="width:100%;height:55px;padding:10px;background:#fafafa;">
	<div style="float:left;padding: 0px">
		<h1><?php echo $salesOrderNumber; ?></h1>
	</div>
	
	<div style="float:right;padding: 0px">
		<div style="float:left;padding: 0px;width:200px">
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" style="width:120px" onclick="printlSalesOrders()">Print</a>
		</div>
		<div style="float:right;padding: 0px;width:300px">
			<a href="javascript:void(0)" <?php echo $approvalButton ?> class="easyui-linkbutton" iconCls="icon-ok" style="width:120px" onclick="approvalSalesOrders()"><?php echo $approvalLabel; ?></a>
...
<div id="tt" class="easyui-tabs" style="width:100%" data-options="tabPosition:'top', border: false, fit: true,tabWidth:200">
	<div title='Sales Orders Details' style='padding:0px'>
					<!-- Content -->
						<div id="p" class="easyui-panel"
								...
								<iframe border="0" src="<?php echo base_url('salesorders/pdfSalesOrders/' . $action); ?>" width="100%" height="100%"></iframe>
						</div>
					<!-- Content -->
	</div>
	
	<div title='Delivered' style='padding:0px'>
	</div>
